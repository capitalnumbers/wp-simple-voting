...
class WSV_Voting_List_Table extends WP_List_Table {
    ...
    function get_vote_list() {
        ...
        $this->posttype_val = sanitize_text_field($_GET['posttype']);
        $posttype = $this->posttype_val;
    ...
    function set_item_data() {
        for ($i = 0; $i < count($this->vote_list); $i++) {
            ...
            if ($this->posttype_val == "all") {
                $cptobj = get_post_type_object($this->vote_list[$i]->post_type);
                $this->item_data[$i]['post_type'] = $cptobj->labels->singular_name;
            }
